...
export class FireView extends Container {
  private readonly particles: Particle[] = [];
  private time = 0;

  /**
   * Called when the scene (controller) becomes active.
   * Sets up sprites on first run and reattaches/resets them on subsequent runs.
   */
  initialize(): void {
    this.time = 0;

    if (this.particles.length > 0) {
      this.reattachAndResetParticles();
      return;
    }
  ...
  /**
   * Per-frame update for the fire effect.
   */
  tick(delta: number): void {
    ...
    for (const particle of this.particles) {
      if (particle.type === 'glow') {
        ...
        const alphaPulse =
          GLOW_ALPHA_BASE + Math.sin(this.time * GLOW_ALPHA_PULSE_FREQUENCY) * GLOW_ALPHA_AMPLITUDE;
      ...
      if (particle.type === 'core') {
        ...
        const alphaPulse =
          CORE_ALPHA_BASE + Math.sin(this.time * CORE_ALPHA_PULSE_FREQUENCY) * CORE_ALPHA_AMPLITUDE;
        particle.sprite.scale.set(particle.baseScale * scalePulse);
        particle.sprite.alpha = alphaPulse;
        continue;
      }
      ...
      if (particle.type === 'flame') {
        const scaleX = particle.baseScale * (FLAME_SCALE_X_BASE + t * FLAME_SCALE_X_RANGE);
        const scaleY = particle.baseScale * (FLAME_SCALE_Y_BASE + t * FLAME_SCALE_Y_RANGE);
  ...
  /**
   * Detach sprites from the scene while keeping particle data for reuse.
   */
  detachSprites(): void {
    this.removeChildren();
  }
  ...
  /**
   * Position the fire view in the center of the given design size.
   */
  centerIn(designWidth: number, designHeight: number): void {
    this.position.set(designWidth / CENTER_DIVISOR, designHeight / CENTER_DIVISOR);
  }

  private reattachAndResetParticles(): void {
    this.removeChildren();
